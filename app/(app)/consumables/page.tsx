              ...
              {visibleRows.map((row) => (
                <tr
                  ...
                  className={`border-b border-rim/50 transition-colors ${
                    row.isWasted || row.remaining === 0 ? "opacity-40" : "hover:bg-surface-hi/20"
                  }`}
                  style={row.isWasted ? { background: "repeating-linear-gradient(-45deg, transparent, transparent 6px, rgba(245,158,11,0.06) 6px, rgba(245,158,11,0.06) 12px)" } : undefined}
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={row.crafterActive ? "text-ink-dim" : "text-ink-faint"}>{row.crafter.characterName}</span>
                    {!row.crafterActive && (
